...
// prettier-ignore
const { data: searchTags, status } = useAsyncData(() => `search-${q.value}`, fetchAutocomplete, { server: false, watch: [provider] });
async function fetchAutocomplete() {
  const { data, error } = await getApiAutocomplete({ query: { q: q.value } });
  if (data) return data;
  throw error;
}

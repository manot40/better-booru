...
export const useBooruFetch = (
  ...
  const error = useState('booru-fetch-error', () => shallowRef<string>());
  ...
  function dedupe(postList: InternalApiPostItem[]) {
  ...
  async function fetchBooru(state?: InfiScrollState, reset = false as boolean | 'only-data') {
    ...
    loading.value = true;
    const { data: res, error: err } = await getApiPosts({ query: { ...query, page: query.page.toString() } });
    loading.value = false;
...
type Options = {
  onReset?: (data: InternalApiPostItem[]) => void;
  onAppend?: (data: InternalApiPostItem[]) => void;
};

type Result = {
  post: InternalApiPostItem[];

...
import { Bean, CloudLightning, LoaderCircle } from '@lucide/vue';
...
const post = shallowRef<InternalApiPostItem>();
...
<template>
  <EmptyState v-if="error" class="py-8 px-4 h-[80dvh] mt-15" :title="error">
  ...
  <VirtualMasonry
    ...
    <template #end>
      ...
      <EmptyState class="py-8 px-4" v-else-if="loading">
        <template #icon><LoaderCircle class="animate-spin size-10! mb-3!" /></template>
